...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X:np.ndarray):
        ...
        dists[:,:]=(-2*np.dot(X,self.X_train.T)+(X**2).sum(axis=1,keepdims=True)+(self.X_train**2).sum(axis=1))**0.5
        return dists
    ...
    def predict_labels(self, dists:np.ndarray, k=1):
        ...
        for i in range(num_test):
            ...
            #########################################################################
            closest_y_idx=dists[i].argsort()[:k]
            ...
            # label.                                                                #
            #########################################################################
            closest_y_counts=np.bincount(closest_y)
            y_pred[i]=closest_y_counts.argmax()
